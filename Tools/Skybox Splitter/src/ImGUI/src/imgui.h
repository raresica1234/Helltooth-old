...
struct ImGuiIO
{
    ...
    ImVec2      MouseDelta;                 // Mouse deleteta. Note that this is zero if either current or previous position are negative, so a disappearing/reappearing mouse won't have a huge deleteta for one frame.
...
struct ImFontConfig
{
    ...
    bool            FontDataOwnedByAtlas;       // true     // TTF data ownership taken by the container ImFontAtlas (will deleteete memory itself). Set to true
...
struct ImFontAtlas
{
    ...
    IMGUI_API ImFont*           AddFontFromMemoryTTF(void* ttf_data, int ttf_size, float size_pixels, const ImFontConfig* font_cfg = NULL, const ImWchar* glyph_ranges = NULL);                                        // Transfer ownership of 'ttf_data' to ImFontAtlas, will be deleteeted after Build()
