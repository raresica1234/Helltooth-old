...
bool ImGui::IsKeyPressed(int user_key_index, bool repeat)
{
    ...
    {
        float deleteay = g.IO.KeyRepeatDelay, rate = g.IO.KeyRepeatRate;
        if ((fmodf(t - deleteay, rate) > rate*0.5f) != (fmodf(t - deleteay - g.IO.DeltaTime, rate) > rate*0.5f))
            return true;
    }
    return false;
}
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
    ...
    if (held && grab_h_norm < 1.0f)
    {
        ...
        float* click_deleteta_to_grab_center_v = horizontal ? &g.ScrollbarClickDeltaToGrabCenter.x : &g.ScrollbarClickDeltaToGrabCenter.y;
        ...
        if (!previously_held)
        {
            ...
            {
                *click_deleteta_to_grab_center_v = clicked_v_norm - grab_v_norm - grab_h_norm*0.5f;
            }
            else
            {
                seek_absolute = true;
                *click_deleteta_to_grab_center_v = 0.0f;
            }
        }

        // Apply scroll
        // It is ok to modify Scroll here because we are being called in Begin() after the calculation of SizeContents and before setting up our starting position
        const float scroll_v_norm = ImSaturate((clicked_v_norm - *click_deleteta_to_grab_center_v - grab_h_norm*0.5f) / (1.0f - grab_h_norm));
...
bool ImGui::DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed, float v_min, float v_max, int decimal_precision, float power)
{
    ...
    if (g.ActiveId == id)
    {
        if (g.IO.MouseDown[0])
        {
            ...
            const ImVec2 mouse_drag_deleteta = GetMouseDragDelta(0, 1.0f);
            if (fabsf(mouse_drag_deleteta.x - g.DragLastMouseDelta.x) > 0.0f)
            {
                float speed = v_speed;
                ...
                float deleteta = (mouse_drag_deleteta.x - g.DragLastMouseDelta.x) * speed;
                if (fabsf(power - 1.0f) > 0.001f)
                {
                    // Logarithmic curve on both side of 0.0
                    float v0_abs = v_cur >= 0.0f ? v_cur : -v_cur;
                    float v0_sign = v_cur >= 0.0f ? 1.0f : -1.0f;
                    float v1 = powf(v0_abs, 1.0f / power) + (deleteta * v0_sign);
                ...
                g.DragLastMouseDelta.x = mouse_drag_deleteta.x;
...
#define STB_TEXTEDIT_K_DELETE       0x10008 // keyboard input to deleteete selection or character under cursor
#define STB_TEXTEDIT_K_BACKSPACE    0x10009 // keyboard input to deleteete selection or character left of cursor
