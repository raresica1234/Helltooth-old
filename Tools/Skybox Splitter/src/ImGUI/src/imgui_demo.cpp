...
// Message to the person tempted to deleteete this file when integrating ImGui into their code base:
...
void ImGui::ShowTestWindow(bool* p_open)
{
    ...
    if (ImGui::CollapsingHeader("Layout"))
    {
        ...
        if (ImGui::TreeNode("Horizontal Scrolling"))
        {
            ...
            ImGui::EndChild();
            ImGui::PopStyleVar(2);
            float scroll_x_deleteta = 0.0f;
            ImGui::SmallButton("<<"); if (ImGui::IsItemActive()) scroll_x_deleteta = -ImGui::GetIO().DeltaTime * 1000.0f;
            ImGui::SameLine(); ImGui::Text("Scroll from code"); ImGui::SameLine();
            ImGui::SmallButton(">>"); if (ImGui::IsItemActive()) scroll_x_deleteta = +ImGui::GetIO().DeltaTime * 1000.0f;
            if (scroll_x_deleteta != 0.0f)
            {
                ImGui::BeginChild("scrolling"); // Demonstrate a trick: you can use Begin to set yourself in the context of another window (here we are already out of your child window)
                ImGui::SetScrollX(ImGui::GetScrollX() + scroll_x_deleteta);
                ImGui::End();
            }
            ImGui::TreePop();
        }
    ...
    if (ImGui::CollapsingHeader("Popups & Modal windows"))
    {
        ...
        if (ImGui::TreeNode("Modals"))
        {
            ...
            {
                ImGui::Text("All those beautiful files will be deleteeted.\nThis operation cannot be undone!\n\n");
                ImGui::Separator();
    ...
    if (ImGui::CollapsingHeader("Keyboard, Mouse & Focus"))
    {
        ...
        if (ImGui::TreeNode("Dragging"))
        {
            ...
            if (ImGui::IsItemActive())
            {
                ...
                ImVec2 mouse_deleteta = ImGui::GetIO().MouseDelta;
                ImGui::SameLine(); ImGui::Text("Raw (%.1f, %.1f), WithLockThresold (%.1f, %.1f), MouseDelta (%.1f, %.1f)", value_raw.x, value_raw.y, value_with_lock_threshold.x, value_with_lock_threshold.y, mouse_deleteta.x, mouse_deleteta.y);
            }
            ImGui::TreePop();
        }
...
struct ExampleAppConsole
{
    ...
    void    ExecCommand(const char* command_line)
    {
        ...
        // Insert into history. First find match and deleteete it so it can be pushed to the back. This isn't trying to be smart or optimal.
        HistoryPos = -1;
